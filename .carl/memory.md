...
- **Context key identity model**: Semantic context key must be scoped to the durable notebook identity (tab+notebook), not to the runtime epoch (kernel UUID) or transport connection (WebSocket URL). Kernel UUID changes on every Colab transport reconnect; tying the context key to it fragments semantic state across reconnects within the same epoch. Architectural hierarchy: Notebook identity → Runtime epoch (kernel UUID / connection generation) → Transport connections. The kernel UUID should be tracked separately as a diagnostic field to distinguish: transport reconnect (new UUID, no restart signal → definitions persist within epoch), runtime replacement (new UUID, possibly no signal → epoch change, staleness risk), and genuine kernel restart (restart signal → Python state gone, notebook context cleared).
- **Precise persistence guarantee**: Definitions persist across transport reconnects within the same runtime epoch. They do NOT automatically persist across a true kernel restart or an undetected runtime replacement.
